...
def procesar_todo(archivo, modelo, idioma, salida=None):
    """Flujo completo: Extraer (si es video) -> Transcribir -> Guardar -> Limpiar."""
    nombre_base = os.path.splitext(archivo)[0]
    txt_salida = salida if salida else f"{nombre_base}_transcripcion.txt"

    # Extensiones de audio comunes
    extensiones_audio = [".wav", ".mp3", ".flac", ".aac", ".ogg", ".m4a"]

    if os.path.splitext(archivo)[1].lower() in extensiones_audio:
        # Ya es audio, no hace falta FFmpeg
        print(f"--- [1/3] Archivo de audio detectado: {archivo} ---")
        audio_path = archivo
        limpiar = False
    else:
        # Es video, extraemos audio
        audio_path = "temp_audio_ext_whisper.wav"
        extraer_audio_ffmpeg(archivo, audio_path)
        limpiar = True

    try:
        # Paso 2: Transcribir
        texto = transcribir_audio(audio_path, modelo, idioma)
        ...
        # Limpieza solo si se creó un archivo temporal
        if limpiar and os.path.exists(audio_path):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Herramienta para extraer diálogos de video o audio a texto plano."
    )

    parser.add_argument(
        "archivo",
        help="Ruta al archivo de video o audio (mp4, mkv, avi, wav, mp3, etc.)",
    )
    parser.add_argument(
    ...
    if os.path.exists(args.archivo):
        procesar_todo(args.archivo, args.model, args.lang, args.output)
    else:
